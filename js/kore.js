...
async function connect() {
  ...
  if (!isMetaMaskInstalled()) {
    document.getElementById('warning').innerHTML =
      'MetaMask is not installed, thus you cannot interact with the Kore Vault. Please install MetaMask from <a href="https://metamask.com/download.html/">here</a> and try again.';
...
async function loadSummary() {
  ...
  document.getElementById('kore-locked').innerHTML = Number(lockedLP).toFixed(
    5
  );

  var koreBalance = web3.utils.fromWei(
    await koreTokenInstance.methods.balanceOf(address).call()
  );
  document.getElementById('kore-wallet-balance').innerHTML = Number(
    koreBalance
  ).toFixed(5);
  ...
  var claimable = web3.utils.fromWei(
    await vaultV2.methods.pendingKore(0, address).call()
  );
  document.getElementById('kore-claimable-balance').innerHTML = Number(
    claimable
  ).toFixed(5);
}
...
async function loadKVaultData() {
  ...
  var pendingKore = web3.utils.fromWei(
    await vault.methods.pendingKore(0, address).call()
  );
